...
#'
#' Frame rate setting of a video file determines
#' how frequently video screen is refreshed by the next one
#' during its playback.
...
#' that you want to make a detailed inspection on the targeted object
#' in the temporal domain.
#' In this case, you will naturally select to watch the video
#' with a lower frame rate,
#' resulting in a slow-motion view of the recorded object.
#'
...
#' Since the frame rate is determined by a division beween
#' two parameters RATE/SCALE in avi header,
#' this function also receive corresponding two values.
#' This kind of parameter format (representing a quasi-real value by
#' division between two large integer values), by the way,
#' is a kind of tradition in computer science
#' where real numbers are avoided.
#' Do not be afraid of such old convention.
#' Actually, if you simply want an integer frame rate,
...
#' What we want is that, without losing or adding any frames,
...
#' It should be more attentive to use this function
#' only to the copies of vedio files,
...
#' And indeed, changing mere refresh rate during playback,
#' as mentioned above,
#' actually does not have anything to do with original
#' real-time length the frames correspond to.
#' Therefore, leaving this field untouched seems desirable
#' in its actual definition.
#' However, there can be a situation that this functionality comes in handy
#' to correct `usperframe` field improperly set during original recording.
